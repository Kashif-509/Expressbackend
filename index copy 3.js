...
app.put("/api/mineral-revenues/:year/:district", async (req, res) => {
  ...
  try {
    let whereClause = { year, district };

    // Optionally include month in the where clause if it's provided
    if (req.query.month) {
      whereClause.month = req.query.month;
    }
    ...
    if (result[0] === 0) {
      res.status(404).json({
        success: false,
        message: `Mineral revenue data for year ${year}, ${
          req.query.month ? `month ${req.query.month},` : ""
        } and district ${district} not found or no updates applied.`,
      });
    } else {
      res.json({ success: true, message: `Mineral revenue data updated.` });
    }
...
app.put("/api/mineral-revenues/:year", async (req, res) => {
  const { year } = req.params;
  const {
    Limestone,
    Limestone_LSM,
  ...
  try {
    ...
    // Optionally include district and month in the where clause if they're provided
    ...
    if (result[0] === 0) {
      res.status(404).json({
        ...
        message: `Mineral revenue data for year ${year}, ${
          ...
          req.query.district ? `and district ${req.query.district}` : ""
        } not found or no updates applied.`,
      });
    } else {
      res.json({ success: true, message: `Mineral revenue data updated.` });
    }
...
// Update mineral revenue data based on year, district, mineral, and value
